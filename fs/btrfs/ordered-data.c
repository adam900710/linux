				...
				struct folio *locked_folio)
{
	 ...
	 */
	if (locked_folio) {
		u64 range_start = max_t(u64, folio_pos(locked_folio), start);
		u64 range_len = min_t(u64,
				folio_pos(locked_folio) + folio_size(locked_folio),
				end + 1) - range_start;

		ASSERT(folio_test_locked(locked_folio));

		/* The folio should be inside the OE range. */
		ASSERT(folio_pos(locked_folio) <= end ||
		       folio_pos(locked_folio) + folio_size(locked_folio) > start);

		/*
		 * The range must not be dirty, as we should have already submitted it
		 * (writeback) or it's submitted and the folio is released (no flag).
		 */
	 ...
	 */
	if (!test_bit(BTRFS_ORDERED_DIRECT, &entry->flags) && !skip_writeback)
